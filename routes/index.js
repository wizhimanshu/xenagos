...
const fetchNearbyPlaces = async (lat, lon, categories) => {
    if (!GEOAPIFY_API_KEY || GEOAPIFY_API_KEY === 'PASTE_YOUR_GEOAPIFY_API_KEY_HERE') { return []; }
    try {
...
router.post('/search', isAuth, async (req, res) => {
    ...
    try {
        ...
        const [pexelsResult] = await Promise.all([
        ...
        const finalData = {
            ...
            routesInfo: `Accessible by road, rail, and air. Check local travel providers for specific routes.`,
            compareLink: `https://www.skyscanner.co.in/transport/flights-from/in/${encodeURIComponent(locationName)}`,
...
router.get('/local-guide', isAuth, (req, res) => {
    ...
    if (lastSearch && lastSearch.coords) {
        ...
        res.render('local-guide', { hasLocation: false, locationName: null, locationCoords: null });
    }
});
...
router.get('/safety-hub', isAuth, async (req, res) => {
    ...
    if (!OPENWEATHER_API_KEY || OPENWEATHER_API_KEY === 'PASTE_YOUR_OPENWEATHER_API_KEY_HERE') {
        ...
        try {
            const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(location.split(',')[0])}&appid=${OPENWEATHER_API_KEY}&units=metric`);
